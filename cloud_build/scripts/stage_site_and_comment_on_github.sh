...
deploy_to_firebase_staging_channel () {
    ...
    FIREBASE_DEPLOY_RESPONSE=$(firebase hosting:channel:deploy --expires 7d pr$_PR_NUMBER-$HEAD_BRANCH --project=$PROJECT_ID)
...
comment_staging_url_on_github () {
    ...
    # leave a new comment.
    gh pr comment $_PR_NUMBER --edit-last --body-file PR_BODY --repo $REPO_FULL_NAME || \
        gh pr comment $_PR_NUMBER ---body-file PR_BODY --repo $REPO_FULL_NAME
}
